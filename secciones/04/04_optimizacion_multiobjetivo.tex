...
Como fue señalado en la sección 4.2, la función objetivo global se construye como una combinación ponderada de los distintos objetivos clínicos:
\[
F_{\text{tot}}(x)
=
\sum_{m=1}^{M}
...
Una solución \(x^{(1)}\) se dice que domina a otra solución \(x^{(2)}\) si no es peor que \(x^{(2)}\) en ninguno de los objetivos considerados, y es mejor en al menos uno de ellos. Una solución Pareto-óptima es aquella que no tiene ninguna otra solución que la domine. Y al conjunto de todas las soluciones Pareto-óptimas se lo llama el frente de Pareto.
...
Para generar un conjunto representativo de soluciones, se utiliza un conjunto de vectores de pesos distribuidos uniformemente. Dado un parámetro de muestreo \(k\) y una cantidad de objetivos \(M\), el conjunto de vectores de pesos resulta:
\[
\mathcal{W}
=
\left\{
